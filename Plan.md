...
28. **Отправка формы со значением поля `Владелец` меньше допустимой длины и валидными значениями в остальных полях.**
* Заполнить поле `Номер карты` данными `1111 2222 3333 4444`.
* Заполнить поля `"CVC/CVV", "Месяц", "Год"` валидными данными.
* Заполнить поле `Владелец` значением из одной латинской буквы (Например `L`).
* Нажать кнопку **"Продолжить"**.
...
33. **Отправка формы со значением `0` в поле `CVC/CVV`  и валидными значениями в остальных полях.**
* Заполнить поле `Номер карты` данными `1111 2222 3333 4444`.
...
34. **Отправка формы со значением поля `CVC/CVV` меньше допустимой длины и валидными значениями в остальных полях.**
* Заполнить поле `Номер карты` данными `1111 2222 3333 4444`.
* Заполнить поля `"Владелец", "Месяц", "Год"` валидными данными.
...
39. **Успешная оплата `APPROVED`-картой с остальными валидными значениями через** POST **запрос**.
...
40. **Отказ в оплате `DECLINED`-картой с остальными валидными значениями через** POST **запрос**.
...
41. **Отправка формы с несуществующей картой (не `APPROVED`/`DECLINED`) с остальными валидными значениями через** POST **запрос**.
...
**Ожидаемый результат:** Код ответа сервера: `500`:<br/> Тело ответа содержит `"message": "400 Bad Request",`.
